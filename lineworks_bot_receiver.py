...
UPLOAD_STATES = {
    STATE_WAITING_KOBAN, STATE_WAITING_BRANCH_CHOICE, STATE_WAITING_KOBAN_CONFIRM,
    STATE_WAITING_BUHIN, STATE_WAITING_COMMENT, STATE_WAITING_PHASE, STATE_WAITING_BATCH,
}
# アップロード途中のまま放置された会話状態を自動リセットするまでの時間。
# 放置されたまま「工番待ち」が残ると、学習協力の返信まで工番として誤解釈され
# 続けて復帰不能になるため(2026-07-24発生)、一定時間で状態を破棄する。
CONV_STALE_HOURS = 6
...
async def lineworks_callback(request: Request) -> Response:
    ...
    # ── 放置された写真アップロード状態の自動リセット ──────────────────────
    if user_id in _conv:
        _sd = _conv[user_id]
        try:
            _last = datetime.fromisoformat(_sd.get("last_active", ""))
            _stale = (datetime.now(timezone.utc) - _last) > timedelta(hours=CONV_STALE_HOURS)
        except Exception:
            _stale = True  # 記録がない(旧形式)場合はリセットして復旧を優先
        if _stale and _sd.get("state") in UPLOAD_STATES:
            _conv.pop(user_id, None)
            logger.info(f"放置された会話状態をリセット: {user_id} (state={_sd.get('state')})")
    ...
    if user_id in _conv:
        _conv[user_id]["last_active"] = datetime.now(timezone.utc).isoformat()
