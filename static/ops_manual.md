...
- ノート⇔デスクトップの同期確認(2026-07-28〜): 夜間ステータス(task_status/gdx.json)に実行ホストのコミットハッシュと未コミット変更数(dirty)が記録される。ノート側HEADと一致していれば両PCは同一コード。※このためデスクトップではファイルを直接編集しない(push=ノート・pull=デスクトップ)。
