...
Aフォルダ配下に B1〜B4 と F完成({工番}_F完成写真・動画、2026-08-07新設)を用意し、サブフォルダ名を見て振り分ける(判定前に NFKC 正規化)。A直下の写真・動画は B4 へ。F判定はB1〜B4より優先。

| フェーズ | 主な振り分けキーワード |
| --- | --- |
| B1 着手前 | 引取 / 入庫 / 入荷 / 受入 / 入庫時 |
| B2 着手中 | 整備 / 加工 / 切削 / 整備中 / 着手中 |
| B3 出荷以降 | 据付 / 納入 / 出荷 / 引渡 / 搬入 / 出荷時 |
| B4 整理前 | 整理前 / 整理中 / 整理 |
| F 完成時 | 完成 / 完成時 / 完成品 / F完成 (2026-08-07新設。「完成」はB2から移設) |
...
- B4配下に 引取・整備・出荷・完成 等の下位フォルダがあれば再判定し、中身を B1/B2/B3/F へ移す。
...
- run_mie_logged.ps1 の[RESULT]サマリー直前に、[MANUAL](利用マニュアルインデックス)・[GENBA](現場用語カタログ)・[CAL](カレンダースナップショット)・[MITSUMORI](見積インデックス+用語カタログ)・[TATENE](建値表)・[PAREN](全角括弧の半角化、2026-08-06追加)の6工程を配置(各個try/catchで1工程の失敗が他に波及しない)。Stop-Transcript後に置いていた旧構成は失敗が不可視だったため移設した。

### 14-6. ファイル名の命名規約(2026-08-06 制定)

- 見積(231): 「名前_YYMMDD(枝番)」。台帳(シート積み上げ型)は「名前_YYMM～」または「名前_YYMM～MM」。PDFは8桁日付(20YYMMDD)も可。既存5,194件を tools/rename_231_mitsumori.py 等で標準化済み。
...
## 15. FM POST(マイフォト)とFフェーズ（2026-08-07 追加）

### 15-1. FM POST(マイフォト)

- アプリの📸メニュー(app_pages/my_photos.py、全員に表示)。個人領域は Blob `lw-raw/myphoto/<氏名>/<工番>/`。メタ無しのファイルは同期対象外なので個人の置き場として機能し、「格納」で各ファイルの隣に *_meta.json(LW Bot投稿と同一契約)を書くと lw_blob_sync が拾ってZへ取り込む(新しい同期処理は無い)。

- 閲覧は本人+管理者(MYPHOTO_ADMIN_NAMES、既定: 山嵜喜隆・山嵜絵里)。格納前は削除自由、格納後は削除不可。動画は30秒まで(mp4/movのmvhd解析で判定)。コメントは必須(アノテーションとして学習データになる)。

- 未格納アルバム(最終追加から3日経過)は夕方Bot(cmd_evening_reminder)がやさしくお知らせする(警告ではない)。

- 格納済みの個人領域Blobは現状残置(掃除は将来課題。cleanup_blob_photos.py はlw-raw/YYYYMMDD/のみ対象)。

### 15-2. Fフェーズ(完成時)

- 完成品の見せ写真(営業資産)用の新フェーズ。フォルダは `{工番}_F完成写真・動画`(空なら他のBフォルダ同様に自動削除)。時系列のB連番とは独立した「完成ギャラリー」の位置づけ(F=Finish/Free)。

- 対応箇所: Bot投稿のフェーズ選択「F」(※「4」はB4整理前と紛らわしいため不使用)、FM POSTの格納ラジオ、ld_sortの振り分け、91整理(作成・分類・リネーム・空削除)、FMP SEARCHの🏁フィルタ、rag/indexerのphase検出(_F完成)。
...
| 2026-08-07 | FM POST(マイフォト)新設(個人アルバム→コメント付き格納、Blob myphoto/領域+meta契約接続、夕方Bot未格納お知らせ)／新フェーズ「F完成時」({工番}_F完成写真・動画。Bot=F入力・FMP SEARCH🏁フィルタ・「完成」キーワードをB2からFへ移設)／QUOTATION SEARCH+AI Q&Aマニュアル1.6 |
| 2026-08-06 | 見積第2期: 旧xls(xlrd)・PDF(pypdf)取込+複数シートブックの1シート=1見積分割(6,101ファイル/11,400ドキュメント)／レビュー発火をR工番+二段階選択・設問1問に簡素化／QUOTATION SEARCH改善(関連度順・顧客絞り込み・明細全文プレビュー)／命名規約制定と一括標準化5,194件(231日付統一・全角()半角化・)_-残骸修復)／sanitize_name全角()半角化+夜間[PAREN]工程追加／starlette 1.4.0非互換の全ページ500をstarlette<1.4固定で解消 |
