...
Google Drive / 共有Zフォルダ上の写真・動画・資料を、工番を軸に自動で整理・命名・圧縮するプログラムの運用手順をまとめる。対象は現行コードと実運用に一致する内容とする。
...
### 4-1. Google Drive からの取り込み

- Drive の指定親フォルダ直下のフォルダを、GDExtraction へ再帰ダウンロードする。完了した Drive 側フォルダは削除。

- Windows 禁止文字は「_」へ置換、前後空白と末尾「.」は除去、何も残らなければ「untitled」。

- 取り込み後、GDExtraction 直下のフォルダ骨組みを Drive 側へ空フォルダ同期する(ファイルは戻さない)。

### 4-2. 工番マスタによる補正

- GDExtraction 直下の 工事一覧表.csv / CSV工番マスタ.csv / master.csv を参照。

- そろえる対象: 91のAフォルダ名、252_整備資料のAフォルダ名、9781_工事工番のAフォルダ名、92_PO_LISTのファイル名。
...
- Drive を使う処理は credentials.json / token.json の有無を確認。
