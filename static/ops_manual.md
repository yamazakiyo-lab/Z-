...
- 旧リネームの残骸は tools/rename_paren_debris.py で修復済み(計258件): 「)_-」(型式仕様の_切れ、81件)と「)_E」3パターン(E機種サフィックスの_切れ: )_E+数字/)_E_数字→)E_、)_E+日本語→)E、177件)。新パターンを見つけたら同ツールに追加してdry-run→apply。命名の健康診断は tools/survey_231_names.py・tools/survey_zenkaku_and_debris.py を随時実行。
