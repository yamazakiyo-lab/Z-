...
|---|---|---|
| MIE_DailyRun | `<DIR>\run_mie_wrapper.bat` | 毎日 0:00 |
...
```powershell
# 毎日系
schtasks /Create /TN "MIE_DailyRun" /TR "`"C:\Users\user\tseg_vscode\Zフォルダ整理\run_mie_wrapper.bat`"" /SC DAILY /ST 00:00 /F
...
schtasks /Create /TN "TSEG_記念日通知" /TR "`"C:\Users\user\tseg_vscode\Zフォルダ整理\run_anniversary_wrapper.bat`"" /SC DAILY /ST 08:00 /F
```
...
```powershell
foreach($t in 'MIE_DailyRun','LW_Blob_Sync','LW_Send_Morning','LW_Send_Afternoon','LW_Evening_Reminder','TSEG_検索アプリ未利用通知','TSEG_週次利用レポート','TSEG_タスク点検通知'){
