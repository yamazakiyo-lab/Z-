...
class Organizer91:
    ...
    def _remove_dir_if_empty(self, d: Path):
        ...
        for x in items:
            try:
                if x.is_file() and x.name in self.cfg.junk_files:
                    if self.cfg.use_long_path_ops and is_windows():
                        os.chmod(to_long_path(x), stat.S_IWRITE)
                        os.remove(to_long_path(x))
                    else:
                        try:
                            ...
                            pass
                        x.unlink()
                    self.log.info(f"ジャンク削除: {x}")
            except Exception as e:
                self.log.warn(f"ジャンク削除失敗: {x} ({e})")
        ...
        if items:
            self.log.info(f"空ではないため残置: {d} / children={[x.name for x in items]}")
            return
    ...
    def _cleanup_empty_B_folders(self, b_folders: List[Path]):
        if not self.cfg.delete_empty_phase_folders:
            return
        for b in b_folders:
            self._remove_dir_if_empty(b)
    ...
    def process_A(self, a_folder: Path, idx: int = 0, total: int = 0):
        ...
        self._cleanup_empty_B_folders([b1, b2, b3, b4])
