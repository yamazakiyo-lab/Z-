...
>
> 旧Drive受け皿フォルダは `_manual_input`（手動投入口）に改名して再利用する。
> ここに写真フォルダを置くと、夜間ランの [2] 工程で工番マスタ名に整えられ
> 91 の B4 へ自動投入される（SDカード・過去写真などの一括取り込み用）。
> コードは `_manual_input` を優先し、旧名 `_GDExtraction` にもフォールバックする。
